...
The objective in this problem is to maximize the expected total reward, i.e. find $$\theta$$ $$ \theta = \text{arg \ max}_\theta E_{p(s_{1:T}) ; \theta) \lbrack \sum_{t=1}^T r_t \rbrack. $$ Here $$s_{1:t} = x, l_1, a_1, …, x_t, l_t, a_t$$ indicates that the distribution is over the possible interaction sequences. 
